...
void today() {
    ...
    for(auto const& line : lines) {
        ls.emplace_back(line.getRawLine());
        auto x = tokenize(line.getRawLine(), "-");
        // auto x = line.to_string(0);
        
        connected.emplace_back(std::make_pair(x[0], x[1]));
    }
    ...
    for(auto i = 0; i < ls[0].size(); ++i) {
        for(auto j = 0; j < ls.size(); ++j) {
            auto cellValue = ls[j][i];
            grid.setValue(i, j, cellValue - '0');
        }
    }
    */
    ...
    for(auto c : connected) {
        if (c.first == "start") {
            p.emplace_back(std::make_pair(c.first, c.second));
        }
    }
    ...
    for(auto c : connected) {
        if (c.first != "start") {
            ...
            if (c.first != "end")
                ...
                p.emplace_back(std::make_pair(c.second, c.first));
        }
    }
    ...
    do {
        ...
        if (p[0].first != "start") continue;
        
        for(auto i = 1; i < p.size(); ++i) {
            auto pFirst = p[i].first;
            auto pSecond = p[i].second;
